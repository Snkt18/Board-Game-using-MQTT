...
def on_message(client, userdata, message):
    ...
    if message.payload.decode() != "":
        ...
        if topic == topic_death:
            ...
            status_update(msg)
        elif topic.split('/')[2] == "join":
            receivedPlayerID = topic.split('/')[3]
            print(f"\n\tPlayer {receivedPlayerID} has joined")
            player_joined += 1
...
client.publish(topic_join + f"/{current_player_id}", json.dumps({"Game": current_player_id}), retain=True)
while player_joined < Number_Players:
    time.sleep(0.1)
